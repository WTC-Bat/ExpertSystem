...
function req_evaluation($req, array $facts, array $rules)
{
	...
	//parentheses first
	//
	//this function samples the idea of testing negation and then evaluating
	//any other compunding used by the requirement
	if (strpos($req, "(") !== FALSE)
	{
...
/*
**	$req - the requirement straight from the associated 'Rule'
*/
//this function samples the idea of finding requirements in parentheses and
//evaluation them first
function reval($req, array $facts, array $rules) //function req_parse()
{
	//Hold the array of facts in requirement; with initial state
	$rfacts = rfact_array($req, $facts);
	//Will hold the index of the first occurence of '('
	$lpar;
	//Will hold the index of the first occurence of ')'
	$rpar;
	//Will hold the part of the requirement in parentheses,
	//that needs to be evaluated first
	$inireq;
	//Will hold the rest of the requirement (not in parentheses)
	$oreq;
	//Will hold the result of the evaluation of the "parenthesized" requirements
	$parres;

	if (($lpar = strpos($req, '(')) !== FALSE)
	{
		if (($rpar = strpos($req, ')')) === FALSE)
		{
			//Only openeing parentheses found
		}
		$inireq = substr($req, ($lpar + 1), ($rpar - ($lpar + 1)));
		$parres = reval($inireq, $facts, $rules);
	}
